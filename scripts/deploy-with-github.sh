...
echo -e "- GitHub Connection:             ${CYAN}${APP_NAME}-gh-conn${NC}"
...
if [ -n "$CONNECTION_ARN" ]; then
    ...
    if [ "$CONNECTION_STATUS" == "PENDING" ]; then
        ...
        echo -e "3. Find connection: ${CYAN}${APP_NAME}-gh-conn${NC}"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "- GitHub Connection:         ${GREEN}${APP_NAME}-gh-conn${NC}"
    ...
    echo -e "   - Find the connection named '${APP_NAME}-gh-conn'"
