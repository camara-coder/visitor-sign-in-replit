...
echo -e "- GitHub Connection:             ${CYAN}visitor-app-github${NC} (fixed name due to 32-char limit)"
...
if [ -n "$CONNECTION_ARN" ]; then
    ...
    if [ "$CONNECTION_STATUS" == "PENDING" ]; then
        ...
        echo -e "3. Find connection: ${CYAN}visitor-app-github${NC}"
...
if [ $? -eq 0 ]; then
    ...
    echo -e "- GitHub Connection:         ${GREEN}visitor-app-github${NC}"
    ...
    echo -e "   - Find the connection named 'visitor-app-github'"
